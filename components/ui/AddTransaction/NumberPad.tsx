...
export const NumberPad: React.FC<NumberPadProps> = ({ onPressNum, onDelete, onSubmit }) => {
    ...
    <View className="h-[260px] flex-row bg-background">
      ...
      <View className="flex-1 flex-row flex-wrap">
        {keys.map((key) => (
          <TouchableOpacity
            key={key}
            className="w-1/3 h-[65px] bg-card items-center justify-center border-r border-t border-border active:bg-border"
      ...
      <View className="w-1/4 flex-col">
        {/* 账户选择键 */}
        <TouchableOpacity className="flex-1 bg-border items-center justify-center border-t border-border">
          <Text className="text-sm font-medium text-text">账户</Text>
          <Text className="text-[10px] text-textSecondary mt-1">微信</Text>
        </TouchableOpacity>
