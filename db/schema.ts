...
export const accounts = sqliteTable('accounts', {
  ...
  creditLimit: real('credit_limit').default(0.00), // 信用卡额度
...
export const budgets = sqliteTable('budgets', {
  ...
  period: text('period', { enum: ['daily', 'weekly', 'monthly', 'yearly'] }).default('monthly'), // 预算周期
...
export const attachments = sqliteTable('attachments', {
  ...
  uploadedAt: integer('uploaded_at', { mode: 'timestamp' }).default(sql`(strftime('%s', 'now'))`), // 上传时间
});